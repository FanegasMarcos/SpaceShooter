...
public class Inimigos : MonoBehaviour
{
    ...
    public GameObject efeitoDeEsplosao;
    ...
    public float tempoAltualDosLasers;
    ...
    {
        tempoAltualDosLasers -= Time.deltaTime;

        if (tempoAltualDosLasers <= 0)
        {
            Instantiate(laserDoInimigo, localDoDisparo.position, Quaternion.Euler(0f, 0f, 90f));
            tempoAltualDosLasers = tempoMaximoEntreOsLasers;
        }
    }
    public void MachucarInimigo(int danoParaReceber)
    {
        ...
        if(vidaAtualDoInimigo <= 0)
        {
            ...
            Instantiate(efeitoDeEsplosao, transform.position, transform.rotation);
